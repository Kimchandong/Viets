...
버전: v0.2 (설계 초안) · 작성일: 2026-08-26 · 최종수정: 2026-08-26

## 변경 이력

| 일자 | 변경 내용 | 근거 |
|---|---|---|
| 2026-08-26 | §2를 "제약 설명"에서 "확정된 실행 환경 정책"으로 재작성, §6 미결정 목록에서 실행 환경 항목 제거 | DECISIONS.md D24 ACCEPTED |
...
## 2. 실행 환경 정책 (확정 — DECISIONS.md D24, 2026-08-26 ACCEPTED)

이 클라우드 세션의 작업공간은 `registry.npmjs.org` 접근이 403으로 차단되어 있음을 확인했다(Phase 1 착수 시도 중 발견). 이에 따라 실행 환경을 다음과 같이 확정한다.

| 환경 | 용도 | 위 체크리스트 항목 |
|---|---|---|
| Cloud 세션 | 코드 분석, 설계, 코드 작성 및 리뷰 | 해당 없음(정적 분석/코드 리뷰 수준만) |
| 데스크톱 연결 프로젝트 폴더 | 실제 실행 검증 전반 | 1~8번 전부(TypeScript check, Lint, Unit/Integration Test, DB migration test, RLS test, API test, Android/iOS test) |

- `npm install`, Expo 실행, 실제 디바이스 테스트, APK/iOS 빌드 등 **실행 검증은 항상 데스크톱에 연결된 프로젝트 폴더에서 수행**한다.
- 이 세션은 현재 사용자 데스크톱의 프로젝트 폴더가 연결되지 않은 상태다 — 실제 QA 실행은 폴더 연결 이후부터 가능하다.
- QA를 건너뛰고 "완료"로 보고하지 않는다 — 데스크톱 폴더가 아직 연결되지 않아 실행할 수 없는 항목은 "미검증" 상태로 명시하고, 실제로 실행 가능해질 때까지 해당 기능을 완료로 처리하지 않는다.
...
1. E2E 도구(Detox vs Maestro) 선택.
2. Performance 기준치(TTI, API 응답시간 등) 수치화.
3. 대표 기기 매트릭스(Android/iOS 버전, 화면 크기) 목록.

상세 근거/추천안은 DECISIONS.md 참조(각각 D25, D26, D27).

~~4. 실제 코드/QA 실행 환경~~ → **ACCEPTED, §2로 이동** (DECISIONS.md D24)
